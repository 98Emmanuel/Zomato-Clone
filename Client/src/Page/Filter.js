...
class Filter extends React.Component{
    ...
    .then( res => {
        this.setState({ mealtp: res.data.mealtype })
    })
    .catch((err => console.log(err)))
    ...
    .catch(err => {console.log(err)})
     ...
     // POST location API
     handleLocation = (val) => {
        const { lcost, hcost, Cuisine, sort, page } = this.state;
       ...
       handleCost = (lcost, hcost) => {
        const { loca, Cuisine, sort, page } = this.state;
   ...
   handleCuisine = (i) => {
        const { loca, lcost, hcost, sort, page } = this.state;
    ...
    handleSort = (sort) => {
        ...
        const { loca, Cuisine, lcost, hcost, page } = this.state;
    ...
    // handle Page
    handlePage = (page) => {
        const { loca, Cuisine, lcost, hcost, sort } = this.state;
    ...
    render(){
        ...
        const { mealtp, loc, restaurant } = this.state;
        //console.log(mealtp);
        ...
        <div class="container mb-5">
            ...
            <h2 class="filter-heading mt-3">{mealtp.name} Places in Mumbai</h2>
            ...
            <div class="filter-box mt-2 pb-4">
                ...
                <select class="form-control selectLocation" onChange={this.handleLocation}>
                   ...
                   
                    {
                        ...
                        loc?.map(item => {// item is a variable representing each object in the location dataset
                            return(
                ...
                { restaurant.length !== 0 ?
                   restaurant.map((res) => {
                    return(
                        ...
                        <div class="d-flex">
                            <div class="lt-box">
                                <img src={res.thumb} alt='Image1' class="img-fluid img-qs" />
                            </div>
                            <div class="rt-box">
